...
def read_table(
    ...
    if combine_array_columns:
        ...
        for arr_name, arr_size in arr_sizes.items():
            arr_idx = ["{}_{}".format(i, arr_name) for i in range(1, arr_size + 1)]
...
def parse_microstructure(ms_str):
    ...
    pat = (
        r"\[[G|g]rain(\d+)\][\s\S](?:crystallite\s(?:\d+)[\s\S])?\(constituent\)\s+"
...
def parse_texture_gauss(texture_str):
    ...
    pat = (
        r"\[[G|g]rain(\d+)\][\s\S]\(gauss\)\s+phi1\s+(\d+\.?\d+)\s+Phi\s+(\d+\.?\d+)"
...
def read_geom(geom_path):
    ...
    with Path(geom_path).open("r") as handle:
        ...
        if ms_match and (
            np.min(constituent_orientation_idx) < 0
            or np.max(constituent_orientation_idx) > len(orientations["euler_angles"])
        ):
        ...
        size_pat = (
            r"size\s+x\s+(\d+(?:\.\d+)*)" r"\s+y\s+(\d+(?:\.\d+)*)\s+z\s+(\d+(?:\.\d+)*)"
        )
