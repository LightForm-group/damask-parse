...
from damask_parse.utils import zeropad, align_orientations, validate_volume_element
...
def write_material_config(
    ...
    if not homog_labels:
        if len(homog_schemes) > 1:
            msg = (
                f"If `homog_labels` is not specified, there must be only one "
    ...
    for homog_label in homog_labels:
        if homog_label not in homog_schemes:
            msg = (
                f'Homogenization label "{homog_label}" is not defined in '
                f"`homog_schemes`."
            )
            raise ValueError(msg)
    ...
    if bad_inputs:
        msg = (
            "Specify either `volume_element` or specify file paths to the "
    ...
    mat_conf_lns = (
        format_part_name("Homogenization")
        + homog_lns
        + format_part_name("Crystallite")
        + crystallite_lns
        + format_part_name("Phase")
        + phase_lns
    )
    ...
    if volume_element:
        ...
        if ori["unit_cell_alignment"]["x"] != "a":
            msg = (
                f"Orientations unit cell alignment must be DAMASK-compatible: x "
        ...
        if (ori_CS and model_CS) and (ori_CS != model_CS):
            ...
            if texture_alignment_method == "axes_keyword":
                ...
                axes = [
                    ("+" if not OCS_inv[v].startswith("-") else "") + OCS_inv[v]
        ...
        for ori_idx, euler in enumerate(euler_angles):
            ...
            ori_data = {
                "(gauss)": (
                    f"phi1 {euler[0]:8.4f} Phi {euler[1]:8.4f} "
        ...
        for grain_idx, (phase_idx, ori_idx) in enumerate(zip(all_phase_idx, all_ori_idx)):
            ...
            grain_data = {
                "crystallite": "1",
                "(constituent)": (
                    f"phase {phase_idx + 1} " f"texture {ori_idx + 1} " f"fraction 1.0"
                ),
            }
    ...
    for load_case in load_cases:
        ...
        if stress is None:
            ...
            if isinstance(dg_arr, np.ma.core.MaskedArray):
                msg = (
                    "To use mixed boundary conditions, `stress` must be passed as a "
                    "masked array."
                )
                raise ValueError(msg)
            ...
            if isinstance(stress, np.ma.core.MaskedArray):
                ...
                msg = (
                    "`def_grad_rate` or `def_grad_aim` must be component-wise exclusive "
