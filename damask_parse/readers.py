...
def read_HDF5_file(hdf5_path, incremental_data, field_data, operations=None):
    ...
    field_data : dict
        Dictiony discribing the spatial field to extract from the simulation results. Keys:
            fields: list of str
    ...
    field_response = {}
    for field_name in field_data['fields']:
    ...
    # if field_data['include_displacement']:
    #     field_response['displacement'] = []
    field_response['increments'] = []
    ...
    for inc in field_data['increments']: # wildcards/empty?
        ...
        for field_name in field_data['fields']:
            ...
            # ext_data = np.ascontiguousarray(
            #     ext_data.reshape(tuple(sim_data.cells) + ext_data.shape[1:], order='F')
            # )

            field_response[field_name].append(ext_data)

    # convert list of array to arrays
    # index order (inc, tensor_comps, x, y, z)
    for field_name in field_data['fields']:
        field_response[field_name] = np.array(field_response[field_name])
    # if field_data['include_displacement']:
    #     field_response['displacement'] = np.array(field_response['displacement'])
